...
def run_engineering_critic_stage(run_id, planner_output, architect_output, coder_output):
    update_run_status(run_id, "running", "engineering_critic")

    add_event(
        run_id,
        "engineering_critic_started",
        "engineering_critic",
        "Engineering Critic started.",
    )

    critic = EngineeringCritic()
    result = critic.review(
        original_request=get_task_description_for_run(run_id),
    ...
    save_stage_output(run_id, "engineering_critic_output", result_json)

    add_event(
        run_id,
        "engineering_critic_completed",
        "engineering_critic",
        "Engineering Critic completed.",
        result_json,
    )
...
def run_coder_revision_stage(
    ...
    update_run_status(run_id, "running", "coder_revision")

    add_event(
        run_id,
        "coder_revision_started",
        "coder_revision",
        "Coder revision started from Engineering Critic findings.",
        critic_result.to_json(),
    )

    adapter = LLMAdapter()
    revision_prompt = build_coder_revision_prompt(
        task_description=get_task_description_for_run(run_id),
        planner_output=planner_output,
        architect_output=architect_output,
        previous_coder_output=previous_coder_output,
        critic_result=critic_result,
    )
    raw_output = adapter.ask(
        model=DEFAULT_MODELS["coder"],
        system_prompt="You revise generated projects by returning Executor JSON only.",
    ...
    save_stage_output(run_id, "coder_revision_output", normalized_output)
    save_stage_output(run_id, "coder_output", normalized_output)

    add_event(
        run_id,
        "coder_revision_completed",
        "coder_revision",
        "Coder revision completed and sanitized.",
        normalized_output,
    )
...
def build_coder_revision_prompt(
    task_description,
    planner_output,
    architect_output,
    previous_coder_output,
    ...
    return f"""
Revise the generated implementation before execution.

Return ONLY Executor JSON actions.
Do not use markdown.
Do not explain anything.

This is a revision, not a post-execution repair.
Preserve the original engineering intent.

Original task description:
{task_description or "Not available."}
...
Previous Executor JSON:
{previous_coder_output}

Engineering Critic findings:
{critic_result.to_json()}
...
- Never delete failing behavior instead of implementing it.
- Preserve all original requirements.
- Strengthen weak tests with behavioral assertions.
- Return a complete corrected Executor JSON action list.

Generate revised Executor JSON now.
""".strip()
